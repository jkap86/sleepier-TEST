...
export const getRecordDict = (
    ...
    const getLineupCheck = (matchup, league, allplayers, weeklyRankings, projections, schedule, includeTaxi, includeLocked) => {
        ...
        const getOptimalLineup = () => {
            ...
            starting_slots.forEach((slot, index) => {
                ...
                const slot_options = player_ranks_filtered
                    .filter(x => {
                        const slot_option_kickoff = new Date(parseInt(schedule
                            ?.find(matchup => matchup.team.find(t => matchTeam(t.id) === allplayers[x.id]?.team))
                            ?.kickoff) * 1000).getTime()

                        if (includeLocked && slot_option_kickoff < new Date().getTime()) {
                            return false
                        } else {
                            return position_map[slot].includes(allplayers[x.id]?.position)
                        }
                    })
                    .sort(
                ...
                optimal_lineup.push({
                    ...
                    slot_options: slot_options.map(x => allplayers[x.id].full_name)
                })
            })
        ...
        const findSuboptimal = () => {
            ...
            starting_slots.map((slot, index) => {
                ...
                const kickoff = new Date(parseInt(schedule
                    ?.find(matchup => matchup.team.find(t => matchTeam(t.id) === allplayers[cur_id]?.team))
                    ?.kickoff * 1000))
                ...
                const earlyInFlex = matchup.starters?.find((x, starter_index) => {
                    const alt_kickoff = new Date(parseInt(schedule
                        ?.find(matchup => matchup.team.find(t => matchTeam(t.id) === allplayers[x]?.team))
                        ?.kickoff * 1000))
                ...
                return lineup_check.push({
                    ...
                    nonQBinSF: position_map[slot].includes('QB') && allplayers[(matchup?.starters || [])[index]]?.position !== 'QB',
                    slot_options: slot_options,
                    player: allplayers[matchup?.starters[index]]?.full_name,
                    timeslot: timeslot
            ...
            .forEach(league => {
                ...
                league.rosters
                    .forEach(roster => {
                        ...
                        const userLineup = matchup && getLineupCheck(matchup, league, allplayers, rankings, projections[week], schedule[week], includeTaxi, includeLocked)
                        const oppLineup = opponentMatchup && getLineupCheck(opponentMatchup, league, allplayers, rankings, projections[week], schedule[week], includeTaxi, includeLocked)
