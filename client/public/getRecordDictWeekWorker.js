...
self.onmessage = (e) => {
    ...
    const { user, leagues, weeks, includeTaxi, includeLocked, projections, allplayers, schedule, rankings, projectionDict, syncing } = e.data;
    ...
    const getLineupCheck = (matchup, league, allplayers, weeklyRankings, projections, schedule, includeTaxi, includeLocked, user) => {
        ...
        const getOptimalLineup = () => {
            ...
            starting_slots.map((slot, index) => {
                ...
                const slot_options = player_ranks_filtered
                    .filter(x =>
                        position_map[slot].includes(allplayers[x.id]?.position)
                    )
                    .sort(
            ...
            .forEach(league => {
                ...
                league.rosters
                    .forEach(roster => {
                        ...
                        const userLineup = matchup && getLineupCheck(matchup, league, allplayers, rankings, projections[week], schedule[week], includeTaxi, includeLocked, user)
